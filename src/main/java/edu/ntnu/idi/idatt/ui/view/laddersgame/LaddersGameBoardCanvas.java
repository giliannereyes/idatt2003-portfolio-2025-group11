 ...
 * @author Trang Duong
 */
public class LaddersGameBoardCanvas extends Canvas {
  private static final Map<String, TileColorType> ACTION_COLOR_MAPPING = Map.of(
  ...
  /**
   * Constructs a LaddersGameBoardCanvas with the specified width and height.
   *
   * @param width is the width of the canvas.
   * @param height is the height of the canvas.
   */
  public LaddersGameBoardCanvas(double width, double height) {
    super(width, height);
  }
  ...
  /**
   * Draws a connector (line or image) for a ladder/snake
   * from the start tile to its destination tile.
  ...
  private void drawLadder(
        GraphicsContext gc, double startX, double startY, double endX, double endY
  ) {
    ...
    if (len > 0) {
      ...
      gc.setLineWidth(5);
      gc.strokeLine(leftX, startY, leftX + dx * len, startY + dy * len);
      gc.strokeLine(rightX, startY, rightX + dx * len, startY + dy * len);
   ...
   */
  private void drawStarOnTile(
        GraphicsContext gc, double x, double y, double tileWidth, double tileHeight
  ) {
