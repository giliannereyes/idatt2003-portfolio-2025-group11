...
/**
 * Unit tests for {@link GameConfigService}, verifying its behavior when updating and retrieving game configuration.
 *
 * @version 0.1
 * @since 0.1
 * @author Trang Duong
...
public class GameConfigServiceTest {
     ...
     */
    @Test
    void testBuildReturnsSameGameConfig() {
        GameConfig result = service.build();
    ...
    void testUpdateBoardSuccessfully() {
        ...
        assertEquals(newBoard, service.build().getBoard(), "Board should be updated correctly");
    }
    ...
    /**
     * Verifies that updating the player configs with {@code null} throws an {@link IllegalArgumentException}.
     */
    @Test
    ...
    void testUpdatePlayerConfigsWithEmptyList() {
        ...
        assertTrue(service.build().getPlayerConfigs().isEmpty(), "Player configs should be empty after setting to empty list");
    }
